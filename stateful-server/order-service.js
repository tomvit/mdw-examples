...
// local storage for the order; 
// should normally be persistent
var storage = {    
    orders : []
};
...
function processOrder(op, params, session) {
    ...
    if (op == "open")
        if (session.data.open)
            ...
            session.data.order_inx = storage.orders.push([]) - 1;
    ...
    if (op == "add")
        if (session.data.open) {
            if ((v = params.match(".*item=([0-9A-Za-z\-]+).*")) && v.length > 0) {
                storage.orders[session.data.order_inx].push(v[1]);
                response = "Item added!";
    ...
    if (op == "close")
        if (session.data.open) {
            for (var i = 0; i < storage.orders[session.data.order_inx].length; i++)
                response += storage.orders[session.data.order_inx][i] + "\n";
